...
const float spherePos[BALLNUM][2] = { {-2.7f,0} , {+2.4f,1.1f} , {2.0f,0}};
// initialize the color of each ball (ball0 ~ ball3)
const D3DXCOLOR sphereColor[BALLNUM] = {d3d::RED, d3d::RED, d3d::YELLOW};
...
bool Setup()
{
    ...
    g_target_whiteball.setCenter(0, (float)M_RADIUS, -3.5f+0.06f+ 3*g_target_whiteball.getRadius());
    g_target_whiteball.setPower(0, 0);
	
	// create blue ball for set direction
    if (false == g_target_blueball.create(Device, d3d::BLUE)) return false;
    g_target_blueball.setCenter(0, (float)M_RADIUS, -3.5f + 0.06f + 1 * g_target_whiteball.getRadius());
...
bool Display(float timeDelta)
{
	int i=0;
	int j = 0;
    DWORD currTime = timeGetTime(); // 현재 시간 측정 
    timeDelta = (currTime - prevTime) * 0.001f;
    //시간 차이 계산 (초 단위) 
    prevTime = currTime;
  ...
  //      for (i = 0; i < BALLNUM; i++) {
        ...
        g_target_whiteball.hitBy(g_target_blueball);
        for (j = 0; j < BALLNUM; j++) {
            if (g_sphere[j].isNull() == false) {
                if (bool temp = g_target_whiteball.hitBy(g_sphere[j])) {
                    g_sphere[j].destroy();
                }
            }
        }
        ...
        if (g_target_whiteball.isNull() == true) {
            //경기장 밖으로 나갔다면
        }
        else {
	...
	case WM_KEYDOWN:
        {
            ...
            case VK_SPACE:
                if (spaceActivate == 0) {
                    ...
                    D3DXVECTOR3	whitepos = g_target_whiteball.getCenter();
                    ...
                    g_target_whiteball.setPower(0,1);
                    spaceActivate = 1;
                }
				break;
	...
	case WM_MOUSEMOVE:
        {
            ...
            if (LOWORD(wParam) & MK_RBUTTON) {
                ...
                if (LOWORD(wParam) & MK_LBUTTON) {
                    ...
                    D3DXVECTOR3 coord3d_W = g_target_whiteball.getCenter();
                    if ((coord3d.x + dx * (-0.007f) > -3 + g_target_blueball.getRadius() + 0.06f) && (coord3d.x + dx * (-0.007f) < 3 - g_target_blueball.getRadius() - 0.06f)) {
                        g_target_blueball.setCenter(coord3d.x + dx * (-0.007f), coord3d.y, coord3d.z);
                        if (spaceActivate == 0) {
                            g_target_whiteball.setCenter(coord3d_W.x + dx * (-0.007f), coord3d_W.y, coord3d_W.z);
                        }
                    }
                }
				old_x = new_x;
