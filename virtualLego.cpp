...
bool Display(float timeDelta)
{
    ...
    if (Device)
    {
        ...
        for (i = 0; i < 4; i++) {
            g_sphere[i].hitBy(g_target_blueball);  // 흰 공이 파란 공과 충돌하는지 확인
        }
...
LRESULT CALLBACK d3d::WndProc(HWND hwnd, UINT msg, WPARAM wParam, LPARAM lParam)
{
    ...
    case WM_KEYDOWN:
    {
        ...
        // 블루볼 방향키 조정
    case VK_LEFT: {
        D3DXVECTOR3 pos = g_target_blueball.getCenter();
        g_target_blueball.setCenter(pos.x - 0.2f, pos.y, pos.z); // 왼쪽으로 이동
        break;
    }
    case VK_RIGHT: {
        D3DXVECTOR3 pos = g_target_blueball.getCenter();
        g_target_blueball.setCenter(pos.x + 0.2f, pos.y, pos.z); // 오른쪽으로 이동
        break;
    }
